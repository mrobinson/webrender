...
impl TransformOrOffset {
    ...
    fn apply(&self, rect: &LayerRect) -> LayerRect {
        match *self {
            ...
            TransformOrOffset::Transform(transform, _) => transform.transform_rect(&rect),
        }
    }
    ...
    pub fn unapply(&self, rect: &LayerRect) -> LayerRect {
        match *self {
            TransformOrOffset::Offset(offset) => rect.translate(&-offset),
            TransformOrOffset::Transform(_, inverse) => inverse.transform_rect(&rect),
        }
    }
    ...
    fn offset(&self, new_offset: LayerVector2D) -> TransformOrOffset {
        match *self {
            ...
            TransformOrOffset::Transform(transform, _) => {
                let transform = transform.pre_translate(new_offset.to_3d());
                let inverse = match transform.inverse() {
                    Some(inverse) => inverse,
                    None => unreachable!("Expected invertible matrix"),
                };
                TransformOrOffset::Transform(transform, inverse)
            }
        }
    }

    fn update(&self, transform: LayerTransform) -> Option<TransformOrOffset> {
        ...
        let transform = match *self {
            ...
            TransformOrOffset::Transform(ref existing_transform, _) =>
        ...
        Some(TransformOrOffset::Transform(transform, inverse))
    }
}
...
impl ClipScrollNode {
    ...
    pub fn update(
        ...
        if match self.node_type {
            NodeType::ReferenceFrame(ref info) if info.uninvertible => true,
            _ => false,
        } {
            self.update_to_empty_rect();
            return;
    ...
    pub fn update_clip_work_item(
        ...
        //         self.local_clip_rect.translate(&self.coordinate_system_relative_transform));
    ...
    pub fn update_transform(
        ...
        //println!("\t++ node offset: {:?}", self.coordinate_system_relative_transform);
    ...
    pub fn update_transform_for_reference_frame(
        ...
        // whatever local transformation this reference frame provides. This can be combined
        // with the local_viewport_rect to get its position in world space.
        let node_transform = info.resolved_transform
        ...
        self.world_viewport_transform = state.parent_reference_frame_transform
            .pre_mul(&node_transform.with_destination::<LayerPixel>());
        self.world_content_transform = self.world_viewport_transform;

        info.uninvertible = node_transform.determinant() == 0.0;
        if info.uninvertible {
            return;
        }

        // Try to update our compatible coordinate system transform. If we cannot, start a new
        // incompatible coordinate system.
        match state.coordinate_system_relative_transform.update(node_transform) {
...
pub struct ReferenceFrameInfo {
    ...
    /// True if the resolved transform becomes uninvertible.
    pub uninvertible: bool,
}
